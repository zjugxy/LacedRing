...
shared uint presum[MEM_LOCAT];
shared bool reverse[MEM_LOCAT];//true 解析right false 解析left
shared uint ewirevernum[MEM_LOCAT];
uint exvernum;
...
void main(){
    ...
    for(int i = 0;i+threadid<ewirenum;i+=GROUP_SIZE){
        uint wireid = i+threadid;
        uint exnum = LowAnaUint(DesInfo[start+2+wireid/4],wireid%4);
        ewirevernum[wireid] = exnum&0x7F;
        reverse[wireid] = (exnum & 0x80)!=0;
    }

	uint pretemp = 0;
    if(threadid==0){
	for(int i=0;i<ewirenum;i++){
		presum[i] = ewirevernum[i] +pretemp-1;
		pretemp = presum[i];
	}
    }
    //	uint pretemp = 0;
...
//con data
     ...
     for(int i=0;i+threadid<intergeonum*2;i+=GROUP_SIZE){
            ...
            uint idx = GetInterWireByuint8(interwirelocation,i+threadid);
     ...
     for(int i = 0; i+threadid < numvertex - intergeonum;i+=GROUP_SIZE){
         uint wireid = FindWireId(i+threadid,ewirenum);
         uint constart = DesInfo[start+1+ingeostart+wireid];
         uint temp = 0;
         if(wireid!=0)
             temp = presum[wireid-1];
         ...
         if(reverse[wireid]==false)
             idx = GetExterWireByuint8(constart,vertexid+8);
         else
             idx = GetExterWireByuint8(constart,vertexid+8+ewirevernum[wireid]);
    
         uint triid = 2*intergeonum+i+threadid;
    
         if(idx==0xFF){
             gl_PrimitiveIndicesNV[triid*3] = 0xFFFFFFFF;
             gl_PrimitiveIndicesNV[triid*3+1] = 0xFFFFFFFF;
             gl_PrimitiveIndicesNV[triid*3+2] = 0xFFFFFFFF;
         }else{
 ...
 //      uint idx0 = GetInterWireByuint8(interwirelocation,id);
 //      uint idx1 = GetInterWireByuint8(interwirelocation,id+1);
 //      uint idx2 = GetInterWireByuint8(interwirelocation,id+2);
 ...
 //  }
   ...
   for(int i = 0;i+threadid<irrnum*3;i+=GROUP_SIZE){
        uint id = 2*intergeonum+i+threadid;
        uint vid = GetInterWireByuint8(interwirelocation,id);
        gl_PrimitiveIndicesNV[2*3*intergeonum+3*exvernum+i+threadid] = vid;
   }



	if(threadid==0)
			gl_PrimitiveCountNV = intergeonum*2
            +exvernum+irrnum;
}
