...
void main()
{
	...
	if(gl_LocalInvocationID.x<meshdes.primitive_cnt&&gl_WorkGroupID.x==0){

		gl_MeshVerticesNV[gl_LocalInvocationID.x*3].gl_Position = projection*view*model*vec4(position[begin+irc],1.0);
		gl_MeshVerticesNV[gl_LocalInvocationID.x*3+1].gl_Position = projection*view*model*vec4(position[begin+1+irc],1.0);
		gl_MeshVerticesNV[gl_LocalInvocationID.x*3+2].gl_Position = projection*view*model*vec4(position[begin+2+irc],1.0);

		v_out[irc+0].color = meshdes.color;
		v_out[irc+1].color = meshdes.color; 
		v_out[irc+2].color = meshdes.color; 
